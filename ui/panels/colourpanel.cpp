...
ColourRibbon::ColourRibbon(QWidget* parent) : QWidget(parent) {
  ...
  connect(fill_btn_, &QPushButton::clicked, this,
          [this]() { PickColour(fill_colour_, "Choose Fill Colour"); });
  layout->addWidget(fill_btn_);
  layout->addSpacing(config::kPanelSpacing);

  stroke_btn_ = new QPushButton("Stroke Colour", this);
  connect(stroke_btn_, &QPushButton::clicked, this,
          [this]() { PickColour(stroke_colour_, "Choose Stroke Colour"); });
  layout->addWidget(stroke_btn_);
  layout->addSpacing(config::kPanelSpacing);

  layout->addWidget(new QLabel("Stroke Width:", this));
  width_spin_ = new QSpinBox(this);
  width_spin_->setRange(config::kStrokeWidthMin, config::kStrokeWidthMax);
  width_spin_->setValue(stroke_width_);
  connect(width_spin_, &QSpinBox::valueChanged, this,
          [this](int v) { SetStrokeWidth(v); });
  ...
  connect(font_combo_, &QComboBox::currentTextChanged, this,
  ...
  font_size_spin_->setRange(config::kFontSizeMin, config::kFontSizeMax);
  font_size_spin_->setValue(font_size_);
  connect(font_size_spin_, QOverload<int>::of(&QSpinBox::valueChanged), this,
          [this](int v) {
            font_size_ = v;
