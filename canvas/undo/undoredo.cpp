...
void UndoRedoManager::Apply(std::stack<StackEntry>& from,
  ...
  switch (entry.type) {
    case ChangeType::kModify: {
      ...
      to.push({entry.index, std::move(current), ChangeType::kModify});
      shapes[entry.index] = Data2Shape::Convert(entry.data);
      break;
    }
    case ChangeType::kAdd: {
      // Insert shape at index; inverse = kRemove
