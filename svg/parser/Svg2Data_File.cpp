  ...
  SvgParser::ParseXml(content, [&](std::string tag, AttrMap attrs) {
    if (tag == "svg") {
      ...
      doc.elements.push_back(
          {tag, attrs});  // the other things are the shapes. we just store
                          // their tag and attributes for now, and convert to
                          // Shapes later in the shape registry (coz we can
                          // reuse the same loading code that way)
    }
  });
  return doc;
}
