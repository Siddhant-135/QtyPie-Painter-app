...
std::string Vec2Svg::AttrsCommon(const Shape& s) {
  return " fill=\"" + ColorToSvg(s.fillColour) + "\" stroke=\"" +
         ColorToSvg(s.strokeColour) + "\" stroke-width=\"" +
         std::to_string(s.strokeWidth) + "\"";
}

bool Vec2Svg::SaveShapesToSvgFile(
  ...
  for (const auto& sp : shapes) {
    ...
    if (const auto* tb = dynamic_cast<const TextBox*>(s)) {
      out << "  <textbox x=\"" << tb->x << "\" y=\"" << tb->y << "\" width=\""
          << tb->w << "\" height=\"" << tb->h << "\" font-family=\""
          << tb->fontFamily.toStdString() << "\" font-size=\"" << tb->fontSize
          << "\" text=\"" << tb->text_line.toStdString() << "\""
          << AttrsCommon(*tb) << "/>\n";
    } else if (const auto* rr = dynamic_cast<const RoundedRectangle*>(s)) {
      out << "  <rect x=\"" << rr->x << "\" y=\"" << rr->y << "\" width=\""
          << rr->w << "\" height=\"" << rr->h << "\" rx=\"" << rr->cornerRadius
          << "\"" << AttrsCommon(*rr) << "/>\n";
    } else if (const auto* r = dynamic_cast<const Rectangle*>(s)) {
      out << "  <rect x=\"" << r->x << "\" y=\"" << r->y << "\" width=\""
          << r->w << "\" height=\"" << r->h << "\"" << AttrsCommon(*r)
          << "/>\n";
    } else if (const auto* sq = dynamic_cast<const Square*>(s)) {
      out << "  <rect x=\"" << sq->x << "\" y=\"" << sq->y << "\" width=\""
          << sq->side << "\" height=\"" << sq->side << "\"" << AttrsCommon(*sq)
          << "/>\n";
    } else if (const auto* c = dynamic_cast<const Circle*>(s)) {
      out << "  <circle cx=\"" << c->cx << "\" cy=\"" << c->cy << "\" r=\""
          << c->radius << "\"" << AttrsCommon(*c) << "/>\n";
    } else if (const auto* l = dynamic_cast<const Line*>(s)) {
      out << "  <line x1=\"" << l->x1 << "\" y1=\"" << l->y1 << "\" x2=\""
          << l->x2 << "\" y2=\"" << l->y2 << "\"" << AttrsCommon(*l) << "/>\n";
