...
std::vector<std::unique_ptr<Shape>> convertToShapes(const SvgData& data) {
  ...
  for (const auto& el : data.elements) {
    for (const auto& trait : registry) {
      if (el.name != trait.svgTag) continue;

      auto shape = trait.create();
      trait.load(shape.get(), el.attributes);

      if (el.attributes.count("fill")) shape->fillColour = parseColor(el.attributes.at("fill"));
      if (el.attributes.count("stroke")) shape->strokeColour = parseColor(el.attributes.at("stroke"));
