...
#include "../registry/shape_registry.h"
...
std::unique_ptr<Shape> Data2Shape::Convert(const SvgTag& tag) {
  ...
  for (const auto& trait : registry) {
    ...
    if (tag.attributes.count("fill"))
      shape->fillColour = parseColor(tag.attributes.at("fill"));
    if (tag.attributes.count("stroke"))
      shape->strokeColour = parseColor(tag.attributes.at("stroke"));
    if (tag.attributes.count("stroke-width"))
      shape->strokeWidth = std::stoi(tag.attributes.at("stroke-width"));
    return shape;
  }
  return nullptr;
}
