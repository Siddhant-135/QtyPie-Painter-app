...
static int val(const AttrMap& a, const std::string& k) { 
    return a.count(k) ? std::stoi(a.at(k)) : 0; 
}
...
void loadLine(Shape* s, const AttrMap& a) {
    ...
    l->x1 = val(a, "x1"); l->y1 = val(a, "y1");
    l->x2 = val(a, "x2"); l->y2 = val(a, "y2");
    l->updateBoundingBox();
}
...
void loadSquare(Shape* s, const AttrMap& a) {
    ...
    sq->y = val(a, "y");
    sq->side = val(a, "width");
    sq->bbox_x = sq->x; sq->bbox_y = sq->y;
    sq->bbox_w = sq->side; sq->bbox_h = sq->side;
}
